...
// Motion sensors are defined as separate Motion Sensor accessories
struct dscMotionZone : Service::MotionSensor {
  byte zoneGroup, zoneBit;
  SpanCharacteristic *zoneState;
  dscMotionZone(byte zone) : Service::MotionSensor() {
    zoneGroup = (zone - 1) / 8;
    zoneBit = (zone - 1) - (zoneGroup * 8);
    bitWrite(configuredZones[zoneGroup], zoneBit, 1);
    zoneState = new Characteristic::MotionDetected(0);
  }
  ...
  void loop() {
    if (updateZones) {
      ...
      if (bitRead(dsc.openZonesChanged[zoneGroup], zoneBit)) {
        ...
        if (bitRead(dsc.openZones[zoneGroup], zoneBit)) zoneState->setVal(1);
        else zoneState->setVal(0);
      }
      for (byte checkZoneGroup = 0; checkZoneGroup < dscZones; checkZoneGroup++) {
        for (byte checkZoneBit = 0; checkZoneBit < 8; checkZoneBit++) {
          if (bitRead(configuredZones[checkZoneGroup], checkZoneBit) && bitRead(dsc.openZonesChanged[checkZoneGroup], checkZoneBit))
            updateZones = true;
        }
      }
    }
...
// Leak detectors are defined as separate Leak Sensor accessories
struct dscLeakZone : Service::LeakSensor {
  byte zoneGroup, zoneBit;
  SpanCharacteristic *zoneState;
  dscLeakZone(byte zone) : Service::LeakSensor() {
    zoneGroup = (zone - 1) / 8;
    zoneBit = (zone - 1) - (zoneGroup * 8);
    bitWrite(configuredZones[zoneGroup], zoneBit, 1);
    zoneState = new Characteristic::LeakDetected(0);
  }
